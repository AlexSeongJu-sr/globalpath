...
def MST(points):
    ...
    for i in range(n):
        for j in range(i+1,n):
            edgelist.append(Edge(i, j, inte.get_distance(points[i].coordi, points[j].coordi)))
...
def dfs(visited, x, y, group_num):
    visited[x][y]=1
    inte.yellow_group[x][y]=group_num
    for i in direction:
        for j in direction:
            pos = (x+i, y+j)
            if inte.is_in(pos) and visited[x+i][y+j]==0 and inte.color[x+i][y+j]=='y':
